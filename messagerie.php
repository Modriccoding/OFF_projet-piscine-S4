...
$user_id = isset($_SESSION['utilisateur_id']) ? $_SESSION['utilisateur_id'] : $_SESSION['admin_id'];
...
// Ajouter les administrateurs à la liste des utilisateurs
$admin_query = "SELECT id, pseudo, photo_profil, email, nom, bio FROM administrateurs";
...
// Ajouter les administrateurs à la liste des amis (si nécessaire)
$admin_query = "SELECT id, pseudo FROM administrateurs";
$admin_result = $conn->query($admin_query);
while ($row = $admin_result->fetch_assoc()) {
    $is_friend = false;
                ...
                <ul class="list-group">
                    <?php foreach ($friends as $friend): ?>
                        <li class="list-group-item">
                            <a href="messagerie.php?friend_id=<?= $friend['id'] ?>"><?= htmlspecialchars($friend['pseudo']) ?></a>
                        </li>
                    <?php endforeach; ?>
                </ul>
            </div>
            ...
            <?php if ($friend_id): ?>
                <div class="col-md-3">
                    <div class="profile-card">
                        <img src="<?= htmlspecialchars($usernames[$friend_id]['photo_profil']) ?>" alt="Photo de profil">
